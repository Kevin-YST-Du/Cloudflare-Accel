 ...
 * 3. [修复] 解决 Docker Pull 时的 context deadline exceeded (超时) 问题。
 * 4. [机制] 改为 Token 转发模式，由客户端通过 Worker 获取认证。
 *
 * -----------------------------------------------------------------------------------------
 */
...
const DEFAULT_CONFIG = {
  ...
  export default {
    async fetch(request, env, ctx) {
      ...
      // ==============================================================================
      // 【核心修复】 1. 拦截并代理 Token 请求
      // 当 Docker 客户端请求 /token 时，转发给官方 auth.docker.io
      // ==============================================================================
      if (url.pathname === '/token') {
        const tokenUrl = `https://auth.docker.io/token${url.search}`;
        return fetch(tokenUrl, {
          method: 'GET',
          headers: {
            'User-Agent': request.headers.get("User-Agent") || "Docker-Client",
      ...
      // 放宽判定：包含 curl 也视为 Docker 相关请求，方便调试
      const isDockerClient = userAgent.includes("docker") || userAgent.includes("go-http") || userAgent.includes("curl");
  ...
  async function handleDockerRequest(request, url) {
    ...
    try {
      ...
      // ==============================================================================
      // 【核心修复】 2. 拦截 401，重写 WWW-Authenticate 
      // 让 Docker 客户端向 Worker 发起认证请求，而不是直接向 Docker 官方发起
      // ==============================================================================
      if (response.status === 401) {
        let wwwAuth = response.headers.get('WWW-Authenticate');
        if (wwwAuth) {
          // 将官方的 realm 地址替换为 Worker 的地址
          // 例如：realm="https://auth.docker.io/token" -> realm="https://你的域名/token"
          wwwAuth = wwwAuth.replace(/realm="https:\/\/auth\.docker\.io\/token"/, `realm="https://${url.hostname}/token"`);
          
          const newResponse = new Response(response.body, response);
          newResponse.headers.set('WWW-Authenticate', wwwAuth);
          return newResponse;
        }
      }
